...
curl --data '{"this":"is a test"}' --header "Content-Type: application/json" http://localhost:1044
...
class MyServer(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if str(self.path).startswith("/ui"):
            ...
            self.send_header("Content-type", "application/json")
            self.end_headers()
            self.wfile.write(bytes("ERROR: unrecongized request" + "\n", "utf-8"))
