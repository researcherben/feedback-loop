...
class MyServer(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self.wfile.write(
            bytes(
                '<a href="http://localhost:1066">reload page</a> runs next available job\n',
                "utf-8",
            )
        )
        ...
        if current_status == "on":
            self.wfile.write(
                bytes(
                    '      <LI><a href="http://localhost:1066?action=stop">stop</a></LI>\n',
                    "utf-8",
                )
            )
        if current_status == "off":
            self.wfile.write(
                bytes(
                    '      <LI><a href="http://localhost:1066?action=start">start</a></LI>\n',
                    "utf-8",
                )
            )
        self.wfile.write(bytes("    </UL>\n", "utf-8"))
    ...
    def do_POST(self):
        ...
        if "val" in interrupt_message.keys():
            if "pr" in interrupt_message.keys():
                ...
                r = requests.post(
                    met_url,
                    json={"%Y-%m-%d %H:%M:%S": now, "msg": str(interrupt_message)},
                    headers=headers,
                )
