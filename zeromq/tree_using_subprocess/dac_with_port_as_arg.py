...
socket_controller = context.socket(zmq.REP)
socket_controller.bind("tcp://*:"+str(port_as_int))

while True:
    #  Wait for next request from client
    message = socket_controller.recv()
    ...
    if 'bye' in str(message):
        ...
        socket_controller.send(b"completed %s" % message)
    else:
        socket_controller.send(b"launched %s" % message)
